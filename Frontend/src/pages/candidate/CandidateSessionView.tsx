          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-[#64748B]">
                @{session.github_username} · {new Date(session.created_at).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
              </p>
            </div>
            {report && (
                ...
                {report.questionBreakdown.map((q, i) => (
                  <div key={i} className="border border-[#E2E8F0] rounded-xl p-5">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <p className="text-sm font-medium text-[#0D1B2A]">Q{i + 1}: {q.question}</p>
                      <span className={`text-sm font-bold shrink-0 ${scoreColor(q.score)}`}>{q.score}/10</span>
                    </div>
                    <div className="bg-[#F8FAFC] rounded-lg p-3 mb-3">
                      <p className="text-xs text-[#94A3B8] mb-1">Your answer</p>
              ...
              {messages.map((m) => (
                <div key={m.id} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 ${
                    m.role === "assistant" ? "bg-[#EBF1FF] text-[#0052FF]" : "bg-[#F1F5F9] text-[#64748B]"
                  }`}>
                    {m.role === "assistant" ? "AI" : "Me"}
                  </div>
                  <div className={`max-w-[75%] rounded-xl px-4 py-3 text-sm leading-relaxed ${
                    m.role === "assistant" ? "bg-[#F8FAFC] text-[#0D1B2A]" : "bg-[#EBF1FF] text-[#0D1B2A]"
                  }`}>
                    {m.content}
                  </div>
                </div>
