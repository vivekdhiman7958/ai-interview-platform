...
export default function EditRole() {
  ...
  function addQuestion() {
    ...
    setForm({ ...form, custom_questions: [...form.custom_questions, newQuestion.trim()] });
    setNewQuestion("");
  }

  function removeQuestion(index: number) {
    setForm({ ...form, custom_questions: form.custom_questions.filter((_, i) => i !== index) });
  }
    ...
    <div className="min-h-screen bg-[#F8FAFC]" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-sm text-[#64748B] mt-1">Update the interview configuration for this role</p>
        </div>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-xl p-8">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              <label className="block text-sm font-medium text-[#0D1B2A] mb-1.5">Description</label>
              <textarea
                value={form.description}
                onChange={(e) => setForm({ ...form, description: e.target.value })}
                rows={3}
                className="w-full border border-[#E2E8F0] rounded-lg px-3.5 py-2.5 text-sm text-[#0D1B2A] outline-none focus:border-[#0052FF] transition resize-none"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-[#0D1B2A] mb-1.5">Difficulty</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-[#0D1B2A] mb-1.5">Number of questions</label>
                <input
                  type="number"
                  min={1}
                  max={10}
            ...
            <div>
              <label className="block text-sm font-medium text-[#0D1B2A] mb-1.5">
                Custom questions <span className="text-[#94A3B8] font-normal ml-1">(optional)</span>
              </label>

              {form.custom_questions.length > 0 && (
                <div className="flex flex-col gap-2 mb-3">
                  {form.custom_questions.map((q, i) => (
                    <div key={i} className="flex items-start gap-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-3.5 py-2.5">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-[#0052FF] hover:bg-[#0046DD] text-white font-semibold text-sm py-3 rounded-lg transition disabled:opacity-60 mt-2"
            >
